...
def electrode_graph(data):
    ...
    for kk in range(0, 7):
        ...
        data.plot.line(y=label_names[2], ax=ax[kk])

    figpath = '/Users/andrew/Code/Data/EEG/hz_128/users/akavas/figures/'
    fig.savefig(figpath + file_in + 'electrodes_1.png')
    plt.show()
    ...
    fig.savefig(figpath + file_in + 'electrodes_2.png')
    plt.show()
...
def zoom_graph(data):
    ...
    fig, axes = plt.subplots(nrows=steps, ncols=1, figsize=[11, 8])
    plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05)
    fig.suptitle('Zoom on FC5', fontsize=16)

    breaks = np.linspace(0, (len(data)-hz*20), steps)
    for kk in range(0, steps):
        end = int(len(data) - breaks[kk])
        data_z = data[:end]
...
folder_in = '/Users/andrew/Code/Data/EEG/hz_128/users/akavas/lab/19-02-09/'
file_in = 'c6_1s_5s_t0_lab.csv'
